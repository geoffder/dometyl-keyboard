...
module Join : sig
  (** A hull between {!KeyHole.t}s of a column, and thin scads for each of the outer faces
...
(** [make ?join_ax ~n_keys ~curve ~caps key]
    ...
    function [curve], which will position each {!KeyHole.t} according to the index (from 0
    to [n_keys - 1]), before they are joined via {!Scad.hull} between the {!KeyHole.faces}
    along the axis indicated by [join_ax]. The [caps] lookup places a key cap above each
    {!KeyHole.t} corresponding to the index (examples/defaults can be found in
    {!module:Caps}). *)
