...
(** Configuration type for number of steps used to draw the bezier curves that
    form the walls. *)
module Steps : sig
  type t = [ `Flat of int | `PerZ of float ]
  (** [`Flat n] simply indicates [n] steps, [`PerZ mm] specifies that there
      should be a step for every [mm] off the ground the start of the wall is. *)

  val to_int : [< `Flat of int | `PerZ of float ] -> float -> int
  (** [to_int t z]

      Converts to a discrete number of steps. In the case of `Flat, it is simply
      the contained integer and the z value will be ignored. For `PerZ, the
      resulting number of steps will be how many times the wrapped float fits
      into the provided z value. *)
end
...
(** Functions that find the point along the top and bottom edges of the start
    position of the wall closest to the given xy position, and provides a bezier
    edge to the ground starting from there. *)
module EdgeDrawer : sig
  type drawer = Vec3.t -> Edge.t
  type t = { top : drawer; bot : drawer } [@@deriving scad]
...
module Edges : sig
  ...
  (** [of_clockwise_list_exn l] Convert a four element list into a [t]. The
      ordering actually shouldn't just be clockwise, but is assumed to literally
      be: TL, TR, BR, BL. *)
...
type t =
  ...
  ; start : Points.t
        (** Corner points of the {!KeyHole.Face.t} this wall emerged from *)
  ...
  ; edge_drawer : EdgeDrawer.t
        (** Generate {!Edge.t}'s emerging from point along top and bottom
            starting edges of the wall closest to the provided {!Vec3.t} on the
            XY plane. *)
...
val swing_face :
  ?step:float -> Vec3.t -> KeyHole.Face.t -> KeyHole.Face.t * Vec3.t
...
(** [poly_siding ?x_off ?y_off ?z_off ?clearance ?n_steps ?n_facets ?d1 ?d2 ?thickness
    ...
    Generate a {!type:t} using an OpenScad polyhedron, drawn from a set of
    bezier curves from the [side] facing edges of [keyhole]. Optional parameters
    influence the shape of the generated wall:

    - [x_off] and [y_off] shift the target endpoints (on the ground) of the wall
    - [z_off] shifts the second bezier control point in z (positive would lead
      to more arc)
    ...
    - [n_steps] controls the number of points used to draw the wall (see:
      ...
      difficult for the OpenScad engine (CGAL) to close. When this happens,
      either decreasing number of steps (can be done preferentially for short
      walls with `PerZ), or increasing [n_facets] to increase the number of (and
    ...
    - [n_facets] sets the number of polyhedron faces assigned to the outside and
      ...
      vertices spaced between the ends of the wall, leading to a finer
      triangular mesh.
    - [d1] and [d2] set the distance projected outward along the orthogonal of
      the [side] of the [keyhole] on the xy-plane used to for the second and
      third quadratic bezier control points respectively.
    - [thickness] influences the thickness of the wall (from inside to outside
      face)
    - If provided, [eyelet_config] describes the screw/bumpon eyelet that should
      be added to the bottom of the generated wall. *)
...
(** [column_drop ?z_off ?clearance ?n_steps ?n_facets ?d1 ?d2 ?thickness ?eyelet_config
    ...
    the desired column [spacing] is calculated and passed along to
    {!val:poly_siding}. *)

val drop_of_config : spacing:float -> config -> [< `North | `South ] -> int -> t
