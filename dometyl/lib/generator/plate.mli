...
(** [make_thumb ~n_keys ~centre_idx ~curve ~rotate_clips keyhole]

Factored out helper function for creation of the thumb cluster, using the distribution
specification [curve] to place [n_keys] [keyhole]s, around the [centre_idx] key.
[rotate_clips] flags whether to rotate the switch-clips / sockets / etc by 90
degrees. Non-rotated, the clips will be however they are on a usual column
(e.g. oriented top to bottom). You may want to set this according to orientation you
would like to have your caps in, since Mx stems are not necessarily radially symmetric. *)
val make_thumb
  :  n_keys:int
  -> centre_idx:int
  -> curve:'k Curvature.t
...
(** [make ?n_rows ?centre_row ?n_cols ?centre_col ?spacing ?tent ?n_thumb_keys ?thumb_centre
      ?thumb_curve ?rotate_thumb_clips ?thumb_offset ?thumb_angle ?lookups keyhole]
...
- [n_rows] specifies number of rows per column
