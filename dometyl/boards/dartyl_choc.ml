...
let body_lookups =
  ...
  and curve = function
    | 0 ->
      Curvature.(
        curve ~well:(well ~radius:42. (Float.pi /. 5.95) ~tilt:(Float.pi /. 5.)) ())
    | _ ->
      Curvature.(
        curve ~well:(well ~radius:42. (Float.pi /. 6.1) ~tilt:(Float.pi /. 15.)) ())
  and swing = function
    | 2 -> Float.pi /. -48.
    | 3 -> Float.pi /. -23.
    | 4 -> Float.pi /. -20.0
...
let thumb_lookups =
  let curve _ =
    Curvature.(
      curve
        ~fan:(fan ~radius:75. ~tilt:(Float.pi /. 48.) (Float.pi /. 10.2))
